...
void POMSolver::compute_average_images(int camera,
  ...
  for(int n = 0; n < room->nb_positions(); n++) 
    if((*proba_absence)[n] <= global_proba_ignored) {
...
/*
room    save info of test.pom
prior   0.01 x nb_positions
views   nth frame from two camera
proba_presense 
nb_frame number of frame
*/
...
void POMSolver::solve(Room *room,
  ...
  for(int it = 0;(nb_stab < global_nb_stable_error_for_convergence) && (it < global_max_nb_solver_iterations);it++) {
    ...
    for(int i = 0; i < room->nb_positions(); i++) {
      ...
      if(abs(proba_absence[i] - np) > e)
        e = abs(proba_absence[i] - np);
      proba_absence[i] = np;
    }

    if(e < global_error_max)
      nb_stab++;
    else 
      nb_stab = 0;
    ...
    if(convergence_file_format) {
      ...
      for(int p = 0; p < room->nb_positions(); p++) 
        (*proba_presence)[p] = 1 - proba_absence[p];
      for(int c = 0; c < room->nb_cameras(); c++) {
