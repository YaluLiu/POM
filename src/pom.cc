...
int TestWrapper(Room *room,
  Vector<scalar_t> *prior,
  Vector<ProbaView *> *proba_views,
  Vector<scalar_t> *proba_presence,
  int nb_frame,
  char* result_format,
  char* result_view_format) {
  POMSolver solver(room);
  solver.solve(room, prior, proba_views, proba_presence, nb_frame, 0);
  
  char buffer[buffer_size];
  if(strcmp(result_view_format, "") != 0)
    for(int c = 0; c < room->nb_cameras(); c++) 
    {
        pomsprintf(buffer, buffer_size, result_view_format, c, nb_frame, 0);
        //pomsprintf(buffer, buffer_size, "./results/result-f%f-c%c.png", c, nb_frame, 0);
        if(true)
  ...
  {
    pomsprintf(buffer, buffer_size, result_format, 0, nb_frame, 0);
    //pomsprintf(buffer, buffer_size, "./results/proba-f%f.dat", 0, nb_frame, 0);
...
int MainWork(char* file_path){

  ifstream *configuration_file = 0;
  istream *input_stream;

  configuration_file = new ifstream(file_path);
  if (configuration_file->fail()) {
    cerr << "Can not open " << file_path << " for reading." << endl;
    exit(1);
  }
    ...
    if(strcmp(token, "ROOM") == 0) {
      ...
      for(int f = first_frame; f < first_frame + nb_frames; f++) {
        ...
        for(int c = 0; c < room->nb_cameras(); c++) {
          ...
          std::cout << "now solve with " << buffer << std::endl;
          tmp.read_png(buffer);
        ...
        TestWrapper(room, &prior, proba_views, &proba_presence, f, result_format,result_view_format);
      }
    }
